...
import { selectItem, selectItemFetch } from 'state/actions/SelectActions';
...
class SelectContainer extends Component {
  ...
  componentDidMount() {
    this.props.selectItemFetch();
  }

  componentDidUpdate(prevProps) {
    if (this.props.muscles !== prevProps.muscles) {
      this.setState({
        values: this.props.muscles
      });
    }
  }
  ...
  renderValues() {
    ...
    if (values.length === 0) {
      return <Placeholder className="placeholder">{placeholder}</Placeholder>;
    }
    ...
    return <ValueItem className="value">{values[0]}</ValueItem>;
  }
  ...
  renderOptions() {
    ...
    return (
      <Options className="options">{options.map(this.renderOption)}</Options>
    );
  }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Selection className="selection" onClick={this.onClick}>
          {this.renderValues()}
...
const connectApp = {
  ...
  mapDispatchToProps: dispatch => ({
    ...
    selectItemFetch: () => dispatch(selectItemFetch())
  })
};
...
export const SelectGroup = ConnectAppContainer;
